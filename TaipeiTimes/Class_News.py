...
class Section():
    ...
    def find_article_urls(self, soup):
        """creates a list of article urls"""
        article_urls = []
        all_urls = soup.find_all(resource=re.compile(self.base)) #customize
        ...
        print (self.title + " has " + str(len(article_urls)) + " articles")
        return article_urls

    def create_article_objects(self, article_urls):
        """create article objects with article urls
           output: a list of article objects as an attribute"""
        for url in article_urls: #from list of article urls
